...
class core_widgets_displayextras extends WP_Widget {  
    function core_widgets_displayextras() {
...
class core_widgets_googlemap extends WP_Widget { // UPDATED 6TH JULY
    ...
    function widget($args, $instance) {
		...
		global $CORE, $wpdb, $category; $STRING = ""; @extract($args); 
	 	...
	 	wp_enqueue_script( 'googlemap', 'https://maps.googleapis.com/maps/api/js?v=3.exp&signed_in=true' );
...
class Bootstrap_Walker extends Walker_Nav_Menu {     
        ...
        function start_lvl(&$output, $depth = 0, $args = array()) 
        {
			...
			if(!isset($GLOBALS['flasg_smalldevicemenubar'])){ $mname = "dropdown-menu"; } else { $mname = "smalldevice_dropmenu"; }
				...
				if ( ( $depth == 0 || $depth == 1 ) ) { //really, level-1 or level-2, because $depth is misleading here (see note above) 
					...
					$output .= "\n{$tabs}<ul class='dropdown-menu responsive-set'>\n"; 
				}
        ...
        {    
            ...
            $class_names = join( ' ', apply_filters( 'dropdown', array_filter( $classes ), $item ) ); 
            ...
            if ($item->hasChildren && $depth == 0 && !isset($GLOBALS['flasg_smalldevicemenubar']) ) { 
                $item_output .= '<a class="dropdown-toggle" data-toggle="dropdown"'. $attributes .'>';	 //  data-toggle="dropdown"			
				 
            } else { 
                $item_output .= '<a id="hello"'. $attributes .'>'; 
            }
	  		$iconpack = false;	
