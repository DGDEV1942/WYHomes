...
function gateway_paypal($data=""){
	...
	// 2 DECIMAL PLACES
	$GLOBALS['total'] = number_format($GLOBALS['total'],2);
...
function core_generic_gateway_callback($orderid, $orderdata ){ global $wpdb, $CORE, $userdata; $order_data_description = "";
	...
	// CHECK IF THIS IS A SELLSPACE PAYMENT
	}elseif(substr($obits[0],0,3) == "BAN"){
		
		// CREATE A NEW CAMPAIGN
		$my_post = array();
		$my_post['post_title'] 		= $orderid;
		...
		add_post_meta($POSTID, 'campaign', $obits[1]);	
		add_post_meta($POSTID, 'listing_expiry_date', date("Y-m-d H:i:s", strtotime( date("Y-m-d H:i:s") . " +30 days")) );
 
		
	}elseif(substr($obits[0],0,3) == "REW" || substr($obits[0],0,3) == "SUB"){ // CHECK FOR RENEWAL PAYMENT
			...
			// CLEAN OUT RENEWAL EMAIL AND REMINDERS
			update_post_meta($obits[1],'email_sent_30dayreminder','');
			update_post_meta($obits[1],'email_sent_15dayreminder','');
			update_post_meta($obits[1],'email_sent_1dayreminder','');
		 
			if(is_numeric($packageID)){
			$packagefields 		= get_option("packagefields");
			
				$i=1;
				foreach(array('featured', 'html', 'visitorcounter', 'topcategory', 'showgooglemap') as $n){
	...
	// HOOK FOR EXTRAS
	hook_callback_process_orderid_after($obits);
}
...
if(isset($_POST['custom'])  && ( isset($_POST['payment_status']) || isset( $_POST['txn_type'] ) ) ){
