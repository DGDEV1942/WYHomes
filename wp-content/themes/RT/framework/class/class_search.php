...
<div class="btn-group pull-right" style="margin-top: 5px;    margin-right: 15px;">
  ...
  <ul class="dropdown-menu asw-elements">
	...
	global $CORE;
        ...
        ?>
        <li class="postbox adsearchbox closed">
                ...
                <div class="controls span7">
                  ...
                  <select data-placeholder="Choose a value..." class="chzn-select" name="asw_key[]" style="width:300px; font-size:12px;">
                    ...
                    <?php if(defined('WLT_DEALER')){ ?>
                     <option value="ctype" <?php if( $meta_key == "ctype"){echo "selected=selected"; } ?> >Car Types (*Recommended*)</option>
                     <option value="cstatus" <?php if( $meta_key == "cstatus"){echo "selected=selected"; } ?> >Car Status (*Recommended*)</option>
                      <?php } ?>
                      
                      
                      <?php if(defined('WLT_DATING')){ ?>
                     <option value="daseeking" <?php if( $meta_key == "daseeking"){echo "selected=selected"; } ?> >I'm a(*Recommended*)</option>
                     <option value="dagender" <?php if( $meta_key == "dagender"){echo "selected=selected"; } ?> >Looking for (*Recommended*)</option>
                     <option value="daage" <?php if( $meta_key == "daage"){echo "selected=selected"; } ?> >Age (*Recommended*)</option>
                     
                     
                      <?php } ?>
                      
                      <option value="map-country1" <?php if( $meta_key == "map-country1"){echo "selected=selected"; } ?> >Country Search &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; (USER LIST)&nbsp;&nbsp; (*Recommended*)</option>
                       <option value="map-country" <?php if( $meta_key == "map-country"){echo "selected=selected"; } ?> >Country Search  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; (FULL LIST)</option>
                     
                     <option value="">---------------</option>
                      <option value="map-state" <?php if( $meta_key == "map-state"){echo "selected=selected"; } ?>>State Search &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; (LINKED)&nbsp;&nbsp; (*Recommended*)</option>
                      <option value="map-state1" <?php if( $meta_key == "map-state1"){echo "selected=selected"; } ?>>State Search &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; (NOT LINKED)</option>
                     
                     <option value="">---------------</option>
                      <option value="map-city" <?php if( $meta_key == "map-city"){echo "selected=selected"; } ?>>City Search &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; (LINKED)&nbsp;&nbsp; (*Recommended*)</option>
...
function build_form( $fields = null, $form_id = '' ) {
		...
		$flagHeading = false; $headingCount  = 0; $fieldsCounter = 0;
			...
			<a data-toggle="collapse" data-parent="#accordion" href="#head_group_'.$headingCount.'"><i class="glyphicon glyphicon-chevron-down pull-right"></i> </a> 
			
			'.stripslashes($field->label).'</label>
			...
			<div id="head_group_'.$headingCount.'" class="panel-collapse '.$status.'"> <div class="panel-body"> ';
  			...
  			continue; // continue to stop showing the label as well
				...
				if(isset($GLOBALS['CORE_THEME']['layout_columns']['3columns']) && $GLOBALS['CORE_THEME']['layout_columns']['3columns'] == "1"){ $col1 = "col-md-12"; $col2 = "col-md-12"; }else{ $col1 = "col-md-12"; $col2 = "col-md-12";  }
				...
				echo '<select name="'.$field_name.'" class="form-control  "><option value="">&nbsp;&nbsp;</option>';
				...
				$field_name =  $field->key."--".$counter;
                $selected = isset( $_GET[$field_name] ) ? $_GET[$field_name] : '';
				 printf( '<select name="%s" class="form-control  " id="core_country_dropdown'.$extraID.'"><option value="">&nbsp;&nbsp;</option> ', $field_name );
                ...
                $selected = isset( $_GET[$field_name] ) ? $_GET[$field_name] : '';				 
					...
					printf( '<select name="%s" class="form-control  " id="core_state_dropdown'.$extraID.'"><option value="">&nbsp;&nbsp;</option>', $field_name );                 
                	echo "</select><script> 
                ...
                $selected = isset( $_GET[$field_name] ) ? $_GET[$field_name] : '';				 
					...
					printf('<select name="%s" class="form-control  " id="core_city_dropdown'.$extraID.'"><option value="">&nbsp;&nbsp;</option>', $field_name );				
					...
					printf( '<select name="%s" class="form-control  " id="core_'.$field->key.'_dropdown'.$extraID.'"><option value="">&nbsp;&nbsp;</option> ', $field_name );
					echo $CSTR;
					echo '</select>';
					...
					printf( '<input type="text" name="zipcode" value="%s"  class="form-control " />', strip_tags($_GET['zipcode']), '' );
    ...
    return date.valueOf() <= checkin.date.valueOf() ? \'disabled\' : \'\';
						...
						if($field->operator == "NUMERIC"){ echo '<span class="input-group-addon">'.$GLOBALS['CORE_THEME']['currency']['symbol'].'</span></div>'; } // && strpos($field_name,"price") !== false 
							...
							if($field->values == ""){
								...
								printf( '<select name="%s[]" class="form-control " multiple=multiple>', $field_name );
							...
							printf( '<select name="%s" class="form-control ">', $field_name );
                    ...
                    case 'taxonomy':
                        ...
                        $args = array(
							...
							'class' => 'form-control ',
                            'echo' => false,						 
                            'show_option_all' => sprintf( ' ', $ll ), //'-- All %s --' 
                        ...
                        if ( isset($taxo->hierarchical) && $taxo->hierarchical ) {
                            ...
                            if ( $field->field_type == 'multi' ) {
								...
								$dropdown = str_replace(sprintf("<select name='%s'", $field_name), sprintf("<select name='%s[]' multiple='multiple' class='form-control '", $field_name), $dropdown);
							...
							$terms = get_terms($field->key,"orderby=count&order=desc&pad_counts=1");							 
							 $count = count($terms);
							 if ( $count > 0 ){
								 ...
								 foreach ( $terms as $term ) {								 
								   ...
								   $checkedm = "";
									...
									$checkedm = "checked=checked";	
						 			...
						 			if(isset($GLOBALS['CORE_THEME']['layout_columns']['3columns']) && $GLOBALS['CORE_THEME']['layout_columns']['3columns'] == "1"){ $ss = "col-md-9 col-sm-9"; }else{ $ss = "col-md-10 col-sm-10"; }
								   ...
								   echo '<div class="row">
										...
										if(isset($GLOBALS['CORE_THEME']['layout_columns']['search']) && $GLOBALS['CORE_THEME']['layout_columns']['search'] != 4){ 
										echo ' <span>('.$term->count.')</span> <a href="'.get_term_link($term). '"><i class="fa fa-angle-double-right pull-right"></i></a>';
										}
							...
							if ( $field->link != '' ) {
								...
								$dropdown = str_replace(sprintf("<select name='%s' ", $field_name), sprintf("<select name='%s[]' class='form-control ' disabled=disabled ", $field_name), $dropdown);
								...
								$dropdown = "".str_replace(sprintf("<select name='%s' ", $field_name), sprintf("<select name='%s[]' class='form-control ' onChange=\"jQuery('#core_ads_wrapper_".$field->link."').html('<img src=".FRAMREWORK_URI."img/loading1.gif>');ChangeSearchValues('".str_replace("http://","",get_home_url())."',this.value,'".$field->link."','tx_".$field->link."[]','','','core_ads_wrapper_".$field->link."')\" ", $field_name), $dropdown);
                            ...
                            printf( '<input type="text" name="%s" value="" class="form-control " />', $field->key );
                        }
                    ...
                    case 'search':
                        printf( '<input type="text" name="s"  value="%s" class="form-control " />', get_search_query() );
                        break;
		...
		if(isset($GLOBALS['CORE_THEME']['layout_columns']['3columns']) && $GLOBALS['CORE_THEME']['layout_columns']['3columns'] == "1"){ $ss = "col-md-9 col-sm-9"; }else{ $ss = "col-md-5 col-sm-5"; }
		...
		// STAR RATING SYSTEM
			...
			echo '<div class="clearfix"></div><div class="panel1 panel-default1 starratingpanel ">
