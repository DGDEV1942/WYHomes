	 ...
	 $spaceleft = (stripslashes($sellspacedata[$key."_max"]) - $tt[0]->total );
	  ...
	  ?>
        <?php }else{ ?>
          ...
          <a href="<?php echo site_url('wp-login.php', 'login_post'); ?>"  class="btn btn-success btn-lg"><?php echo $CORE->_e(array('auction','55')); ?></a> 
        <?php } ?>
...
return $output;
	...
	function wlt_shortcode_dcats($atts, $content = null){ global $wpdb; $STRING = ""; $count = "";
		...
		foreach ($categories as $category) { 
			...
			$STRING .= '<li class="list-group-item">'.$icon.' <a href="'.get_term_link($category).'">'.$category->name.'</a> '.$count.' </li>';
	...
	$STRING = '<script src="https://maps.googleapis.com/maps/api/js?v=3&sensor=false&amp;language='.$GLOBALS['CORE_THEME']['google_lang'].'&amp;region='.$GLOBALS['CORE_THEME']['google_region'].'" type="text/javascript"></script>';
...
function wlt_page_listings( $atts, $content = null ) {
	...
	if(isset($GLOBALS['flag-home']) && isset($_GET['pd']) && is_numeric($_GET['pd']) ){
	 $paged = $_GET['pd'];
	}else{
   ...
   if(strlen($query) > 1 ){
	...
	switch($custom){
		...
		/*if(defined('WLT_COUPON')){	
			...
			if($term_name != ""){
				$args = array('post_type' => THEME_TAXONOMY.'_type', 'taxonomy' => 'store', 'term' => $term_name, 'showposts' => 10, 'paged'  => $paged, ); 
			}else{
		 ...
		 $args = array( 'post_type' => THEME_TAXONOMY.'_type', 'posts_per_page' => 10, 'orderby' => 'ID', 'order' => 'desc', 'paged'  => $paged, );
	...
	// HIDE EXPIRED
	if(isset($GLOBALS['CORE_THEME']['hide_expired']) && $GLOBALS['CORE_THEME']['hide_expired'] == 1 && is_array($args) ){	 
	...
	if($nav){
		...
		$STRING .= '<ul class="pagination wlt_shortcode_listings_pagenav">'.str_replace("<span","<li><span",str_replace("span/>","span/></li>",str_replace("a/>","a/></li>",str_replace("<a","<li><a rel='nofollow'",paginate_links( array(
			'base' 		=> $base,
			'format' 	=> $format,
			'current' 	=> max( 1, $paged ),
...
function SHOW_CONTACTFORM($style=0){ global $CORE, $post; $STRING = ""; $rn1 = rand("0", "9"); $rn2 = rand("0", "9");	
	...
	function wlt_shortcode_timeleft( $atts, $content = null ) {
		...
		if(defined('WLT_AUCTION')){	
		$run_extrab = "jQuery('#auctionbidform').hide();";
		}
		...
		return "<span id='timeleft_".$postid.$milliseconds."'></span>
			...
			jQuery('#timeleft_".$postid.$milliseconds."').countdown({until: date1, onExpiry: WLTvalidateexpiry".$postid."".$layout_code." });
			});
			...
			</script>";	 //, timezone: ".get_option('gmt_offset')."
	}	
	/* =============================================================================
		[FIELDS] - SHORTCODE
		========================================================================== */	
			...
			case "10": {
            	...
            	<div class="rating '.$ratingstyle.'"> 
                	...
                	<div class="b1">'.round($good_per,0).'<span>%</span></div>
             		...
             		<div class="b3 '.$ncc.'">'.$total.' '.$CORE->_e(array('coupons','28')).'</div> 
