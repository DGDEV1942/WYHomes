...
class white_label_themes { 
	...
	function constants(){ 	
		...
		// THEME VERSION 
		define("THEME_VERSION", "8.9");		
		// RELEASE DATE
		define("THEME_VERSION_DATE", "17th Jan, 2016");		
		// THEME INSTALL LINK
	...
	return 'https://maps.googleapis.com/maps/api/js?language='.$lang.'&amp;region='.$region.$extra;
		...
		if(isset($_GET['zipcode']) && ( strlen($_GET['zipcode']) > 2 && strlen($_GET['zipcode']) < 9 ) ){	 
			...
			if($range > 0){
				...
				if(isset($saved_searches[$thisZip]) && strlen($saved_searches[$thisZip]['log']) > 1 && strlen($saved_searches[$thisZip]['lat']) > 1){			
					...
					$geocode = file_get_contents('http://maps.google.com/maps/api/geocode/json?address='. urlencode($thisZip.$extra) .'&sensor=false&region='.$region.'&language='.$lang.'');
...
function CUSTOMHEADER(){ global $wpdb, $pagenow; $STRING = ""; 
	...
	// DEMO MODE
	if(defined('WLT_DEMOMODE') && !$this->isMobileDevice() ){
  		
		if(!isset($_SESSION['skin']) ){
			wp_register_style( 'demomode',  FRAMREWORK_URI.'css/css.demo.css');	
			wp_enqueue_style( 'demomode' );	
		}
		// LOAD IN CONTENT
		add_action('hook_wrapper_before', 'WLT_DEMOOPTIONS');
		
		if(!isset($_SESSION['skin']) ){	
		
			$carr = array('pink','red','green','blue','grey', 'purple','yellow');
			foreach($carr as $c){		
			wp_register_style( '__'.$c.'__X2',  get_template_directory_uri().'/templates/'.$GLOBALS['CORE_THEME']['template'].'/alternative/'.$c.'.css');	
			wp_enqueue_style(  '__'.$c.'__X2' );
			}		
			// fix alternative
			add_filter( 'style_loader_tag', array($this, 'make_stylesheet_alt' ) );
...
function HITCOUNTER($post_id, $output = false){ global $post, $wpdb;
 	...
 	if( !isset($GLOBALS['CORE_THEME']['hitcounter']) || ( isset($GLOBALS['CORE_THEME']['hitcounter']) && $GLOBALS['CORE_THEME']['hitcounter'] != 1 ) ){
		...
		if(isset($data[$date_now]) && isset($data[$date_now][$user_ip])){
			...
			if( $user_ip != "127.0.0.1" && strlen($user_ip) > 4 && WLT_CACHING == false ){ // block local host calls	
	...
	***/
	...
	if($showFallback){
		if(strpos($fimage, "wlt_thumbnail") !== false) {
			...
			$STRING .='<script type="text/javascript" charset="utf-8"> jQuery(document).ready(function(){ jQuery("a[data-gal^=\'prettyPhoto\']").prettyPhoto();});</script>';
		}		
	}else{
	 	if($showSingleImage){
			$STRING .= "<a href='".$fimage."' data-gal='prettyPhoto'>".$image." <div id='mousetrap'></div> </a>";			
...
function SELLSPACE($type=1, $userid="", $size=""){ global $wpdb;
	...
	case "3": {  // return user banners
		...
		$SQL = "SELECT ".$wpdb->posts.".* FROM ".$wpdb->posts."
				...
				WHERE ".$wpdb->posts.".post_status= 'publish' LIMIT 0,50";	
 ...
 if($GLOBALS['CORE_THEME']['google'] != 1){ return; }
	...
	if( is_array($mapdata) ) {	
		...
		foreach($mapdata as $map){
			...
			"long" 	=> $long, 
			"url"  	=> $permalink,
			"title" => esc_html(str_replace("'","",substr(strip_tags($map->post_title),0,20))),
