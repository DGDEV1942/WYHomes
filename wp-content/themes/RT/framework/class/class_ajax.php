...
if(isset($_POST['wlt_ajax'])){
	...
	switch($_POST['wlt_ajax']){
		...
		if($media->post_author == $userdata->ID || current_user_can('edit_posts') ){
		...
		?>      
          ...
          <input type="text" 
          value="<?php echo $media->post_title; ?>" 
          onchange="WLTSetImgText('<?php echo str_replace("https://","", str_replace("http://","",get_home_url()));?>', '<?php echo $bg[1]; ?>', this.value, 'core_ajax_callback');" 
          ...
          onchange="WLTSetImgOrder('<?php echo str_replace("https://","", str_replace("http://","",get_home_url())); ?>', '<?php echo $bg[1]; ?>', '<?php echo $bg[0]; ?>', this.value, 'core_ajax_callback');"
			...
			switch($_GET['action']){
				...
				case "SaveRating": {
					...
					if(!is_array($rated_user_ips)){ $rated_user_ips = array(); }
						...
						}elseif(isset($rated_user_ips[$_GET['pid']]) && isset($rated_user_ips[$_GET['pid']]['ips']) && !in_array($user_ip, $rated_user_ips[$_GET['pid']]['ips']) ){
