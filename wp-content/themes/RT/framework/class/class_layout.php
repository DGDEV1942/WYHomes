...
echo '<div id="wlt_google_map_wrapper" style="display:none;"><div id="wlt_google_map"></div><div id="wlt_google_map_after"></div></div>';
}
		...
		if(isset($payment_status) && $payment_status != ""){
			switch($payment_status){
				...
				case "success": { 
					...
					}elseif($sentAlready == date('Y-m-d H')){
			...
			if($payment_status == "success"){			
				...
				$last_email_sent = get_option('email_admin_order_new');
				...
				if(strtotime(date('Y-m-d H:i:s')) > strtotime($last_email_sent) ) {
					...
					// UPDATE LAST SENT
					update_option('email_admin_order_new', date('Y-m-d H:i:s',strtotime(date('Y-m-d H:i:s') . "+1 hour")) );
