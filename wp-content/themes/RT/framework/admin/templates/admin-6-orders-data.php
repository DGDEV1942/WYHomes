...
if (!defined('THEME_VERSION')) {	header('HTTP/1.0 403 Forbidden'); exit; } global $CORE;
	...
	}elseif (strpos($order->order_id, "BAN") !== false) {
		$tt = "<span class='label label-important'> Advertising </span>";			
 	}
...
<?php }else{ ?>
<input type="hidden" class="txt" name="order_tax" value="<?php echo $order->order_tax ?>" >
<input type="hidden" class="txt" name="order_shipping" value="<?php echo $order->order_shipping ?>">
<input type="hidden" class="txt" name="order_total" value="<?php echo $order->order_total ?>" >
<?php }  ?>          
<hr />
<b>Email</b><br />
