...
HTML;
        ...
        if( get_option('sr_show_leadcapture') ) {
            $contact_text = 'Schedule a Showing';
          ...
          <div class="sr-details" style="text-align:left;">
            <p class="sr-details-links" style="clear:both;">
              ...
              <span id="sr-listing-contact">
                <a href="#sr-contact-form-wrapper">$contact_text</a>
              </span>
            </p>
            $gallery_markup
            <script>
            ...
            $remarks_markup
              ...
              <div class="rts_clear"></div>
              </div><!-- sr-details -->
            </div><!--container-->
              $contact_markup
              <div class="sr-more-info-wrapper">
              <div class="container">
              <div class="sr-more-info">
              <section>
                <div class="sr-section-wrapper">
              ...
              <section>
                <div class="sr-section-wrapper">
                  <table>
                        <thead>
                            <tr>
                              <th colspan="3"><h5>Address Information</h5></th>
                            </tr>
                        </thead>
                        <tbody>
                        $address
            ...
            </div><div class="rts_clear"></div>
            $mapMarkup
            <script>$lh_analytics</script>
            </div><!-- sr-more-info-wrapper -->
          </div>
HTML;
    ...
    public static function srResidentialResultsGenerator( $response, $settings ) {
        ...
        */
        ...
        $markerCount = 0;
         $resultsMarkup .= '<div class="list_results_cont">';

        $args = array( 'post_type' =>'Cities', 'posts_per_page' => '10','order'=>'asc'); 
        ...
        $postid = get_the_ID(); 	
       if(strpos( $postname, $city ) !== false)
	   {
        $resultsMarkup .= '<div class="list_results_cities">';
        $resultsMarkup .= '<div class="right-sec"><div class="title"><h3>'.get_the_title($postid).'</h3></div>'; 
        $resultsMarkup .= '<div class="desc">'.wp_trim_words(get_the_content($postid),50).'</div></div>';
        $resultsMarkup .= '<div class="links"><a href="'.get_post_permalink($postid).'"><span>Discover '.get_the_title().' >></span></a></div>';
        $resultsMarkup .='</div>';
		}
		
        endwhile;   
        wp_reset_query();
		if ( 'cities' == get_post_type() ) {
        $resultsMarkup .= '<div class="list_results_header">City Listings</div>';
		}	
		else
		{
		 $resultsMarkup .= '<div class="list_results_header">Search Results</div>';	
		}	
        foreach( $response as $listing ) {
            $listing_uid        = $listing->mlsId;
            ...
            // append markup for this listing to the content
             $resultsMarkup .= <<<HTML
              ...
              <!-- <div class="sr-listing">
               ...
               <div class="sr-listing">  
                <a href="$link" class="sr_listing_link"> 
                <div class="sr_listing_box">
                    ...
                    <div class="sr-primary-data">                     
                        <h4>$address | $city</h4> 
                        <span id="sr-price"><i>$listing_USD</i></span>
                    </div>
                    <div class="sr-secondary-data">
                      <ul class="sr-data-column">                        
                        <li><strong>$bedrooms</strong> Beds</li>
                        <li><strong>$bathsTotal</strong> Baths</li>
                        <li><strong>$area</strong> SqFt</li>
                      </ul>                     
                     <div class="sr-mls-status $mls_status">
                            $mls_status
                      </div>
                    </div>
...
HTML;
        ...
        $resultsMarkup .= '</div><div style="clear:both;"></div>';
...
HTML;
    ...
    public static function srContactFormMarkup($listing) {
        ...
        $markup .= '<div class="sr-contact" id="sr-contact-form-wrapper">';

        ob_start();
        ?>
        <div class="container">
            <div class="sr-contact-row">
            <div class="sr-contact-left">
            <h4>Ready to see this property?</h4>
            <p>Enter your information to the right to be personally introduced to this property.</p>
            <div class="sr-contact-details">
                <img src="http://wyhomesearch.com/wp-content/uploads/2012/06/logo11.png">
        ...
        $markup .= '<input type="text" name="sr-cf-name" placeholder="Your Name" pattern="[a-zA-Z0-9 ]+" value="'
            . ( isset( $_POST["sr-cf-name"] ) ? esc_attr( $_POST["sr-cf-name"] ) : '' ) . '" size="40" required/>';
        $markup .= '</p>';
        $markup .= '<p>';
        $markup .= '<input type="email" name="sr-cf-email" placeholder="Your Email" value="'
            . ( isset( $_POST["sr-cf-email"] ) ? esc_attr( $_POST["sr-cf-email"] ) : '' ) . '" size="40" required />';  
        $markup .= '</p>';
        $markup .= '<p>';
        $markup .= '<input type="text" name="sr-cf-subject" placeholder="Subject" value="'
            . ( isset( $_POST["sr-cf-subject"] ) ? esc_attr( $_POST["sr-cf-subject"] ) : $listingName[0] ) . '" size="40" />';
        $markup .= '</p>';
        $markup .= '<p>';
        $markup .= '<textarea rows="10" cols="35" placeholder="Optional message" name="sr-cf-message">'
    ...
    public static function srContactFormDeliver() {		
        ...
        if ( isset( $_POST['sr-cf-submitted'] ) AND $_SESSION['form_sent']!=1 ) {
		   ...
		   $message = esc_textarea( $_POST["sr-cf-message"] );
			...
			$message  = '<table border="1" cellpadding="18px" align="center">
				<tbody>
					<tr>
					<th>Listing Name</th>
					  <th>Request By</th>
		  ...
		  $headers .= 'From: Wyhome search and Properties <info@wyhomesearch.com>' . "\r\n";  

            // If email has been process for sending, display a success message
            if ( wp_mail( $to, 'Wyhome Schedule Request', $message, $headers ) ) {
                echo '<div></div>';
				$_SESSION['form_sent']=1;
