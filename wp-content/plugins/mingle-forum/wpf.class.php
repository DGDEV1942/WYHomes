...
class mingleforum{
	...
	function go() {
		...
		$this->o .= "<div id='wpf-info'><small>
			".__("Mingle Forum ", "mingleforum")." by <a href='http://cartpauj.icomnow.com'>cartpauj</a><br /> 
		...
		echo $above_forum_ad."<div id='wpf-wrapper'>".$this->o."</div>".$below_forum_ad;
		return ob_get_clean();
	}
	...
	function showthread($thread_id){
		...
		if(!current_user_can('administrator') && !is_super_admin($user_ID) && !$this->is_moderator($user_ID, $this->current_forum))
			$wpdb->query($wpdb->prepare("UPDATE $this->t_threads SET views = views+1 WHERE id = $thread_id"));
	...
	function get_pagetitle($bef_title){
		...
		switch($action){
			...
			case "viewtopic":
				...
				break;
			case "search":
				$terms = $wpdb->escape($_POST['search_words']);
	...
	function setup_menu(){
		...
		if($this->options['forum_use_seo_friendly_urls'])
		{
			$menuitems = array(
								...
								"logout" 	=> "<a href='$this->logout_link'>".__("Log out", "mingleforum")."</a>",
								"profile" 	=> $link,
								"reply" 	=> "<a id='reply_button' href='".$this->get_post_reply_link()."'>".__("Reply", "mingleforum")."</a>",
								"new_topic" => "<a href='".$this->get_addtopic_link()."'>".__("New Topic", "mingleforum")."</a>",
								"feed" 		=> "<a id='rss_button' href='$this->topic_feed_url"."$this->current_thread'>".__("RSS-Feed", "mingleforum")."</a>",
								"sticky" 	=> "<a href='".$this->thread_link.$this->current_thread.".".$this->curr_page."&sticky&id=$this->current_thread'>".__("Sticky", "mingleforum")."</a>",
								"unsticky" 	=> "<a href='".$this->thread_link.$this->current_thread.".".$this->curr_page."&sticky&id=$this->current_thread'>".__("Undo Sticky", "mingleforum")."</a>",
								"closed" 	=> "<a id='close_button' href='".$this->thread_link.$this->current_thread.".".$this->curr_page."&closed=1&id=$this->current_thread'>".__("Close", "mingleforum")."</a>",
								"unclosed" 	=> "<a href='".$this->thread_link.$this->current_thread.".".$this->curr_page."&closed=0&id=$this->current_thread'>".__("Re-open", "mingleforum")."</a>",
		...
		else
		{
			$menuitems = array(
							...
							"new_topic" => "<a href='".$this->get_addtopic_link()."'>".__("New Topic", "mingleforum")."</a>",
							"feed" 		=> "<a id='rss_button' href='$this->topic_feed_url"."$this->current_thread'>".__("RSS-Feed", "mingleforum")."</a>",
							"sticky" 	=> "<a href='".$this->get_threadlink($this->current_thread)."&sticky&id=$this->current_thread'>".__("Sticky", "mingleforum")."</a>",
							"unsticky" 	=> "<a href='".$this->get_threadlink($this->current_thread)."&sticky&id=$this->current_thread'>".__("Undo Sticky", "mingleforum")."</a>",
							"closed" 	=> "<a id='close_button' href='".$this->get_threadlink($this->current_thread)."&closed=1&id=$this->current_thread'>".__("Close", "mingleforum")."</a>",
							"unclosed" 	=> "<a href='".$this->get_threadlink($this->current_thread)."&closed=0&id=$this->current_thread'>".__("Re-open", "mingleforum")."</a>",
		...
		if($user_ID || $this->allow_unreg()){
			...
			switch($this->current_view){
				case FORUM:
					$menu .= "	<td valign='top' class='menu_sub'>{$menuitems['new_topic']}</td>";
					break;
				case THREAD:
					if(!$this->is_closed()){
...
DISABLED THIS IN 1.0.25*/
	...
	function remove_topic(){
		...
		if($this->is_moderator($user_ID, $this->current_forum)){
			...
			if(is_plugin_active('mingle/mingle.php') and is_user_logged_in())
			{
				...
				$myDelID = $wpdb->get_var("SELECT mngl_id from $this->t_threads where id = $topic");
				if ($myDelID > 0)
					$board_post->delete( $myDelID );
			}
			//END DELETE MINGLE ENTRY
			$wpdb->query($wpdb->prepare("DELETE FROM $this->t_posts WHERE parent_id = $topic"));
			$wpdb->query($wpdb->prepare("DELETE FROM $this->t_threads WHERE id = $topic"));
		}else{
			//@ob_end_clean();
			wp_die(__("Cheating, are we?", "mingleforum"));
		}	
	...
	function move_topic(){
		...
		if($this->is_moderator($user_ID, $this->current_forum)){
			...
			$wpdb->query($wpdb->prepare($strSQL));
			//@ob_end_clean();
	...
	function remove_post(){
		global $user_ID, $wpdb;
		$id = $_GET['id'];
		$author = $wpdb->get_var($wpdb->prepare("SELECT author_id from $this->t_posts where id = $id"));
		...
		if($del == "ok"){
			$wpdb->query($wpdb->prepare("DELETE FROM $this->t_posts WHERE id = $id"));
	...
	function sticky_post(){
		...
		$id = $_GET['id'];
		$status = $wpdb->get_var($wpdb->prepare("select status from $this->t_threads where id = $id"));

		switch($status){
			case 'sticky': 
				$wpdb->query($wpdb->prepare("update $this->t_threads set status = 'open' where id = $id"));
				break;
			case 'open': 
				$wpdb->query($wpdb->prepare("update $this->t_threads set status = 'sticky' where id = $id"));
				break;
		}
	}
	...
	function search_results(){
		...
		foreach($results as $result){
			...
			$starter = $wpdb->get_var($wpdb->prepare("select starter from $this->t_threads where id = $result->parent_id"));
				$o .= "<tr>
